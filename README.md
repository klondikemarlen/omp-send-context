...
GitHub pull-request pages keep the richer behavior: the same menu action and shortcut preserve the GitHub title, selected-link/permalink metadata, and `## GitHub` prompt section. Selections from a PR diff also make `Location` an exact GitHub diff permalink such as `#diff-<hash>L8` or `#diff-<hash>R8`, and include the file path and available line/side context; ambiguous locations are omitted. Generic pages use `## Web page` and always retain the page URL.
