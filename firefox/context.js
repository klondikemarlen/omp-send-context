;(function (root, factory) {
  ...
  function isEligiblePageUrl(value) {
    ...
    try {
      ...
      return ![...FIREFOX_RESTRICTED_HOSTS].some(
  ...
  function extractGithubDiffLocation(document, selection) {
    if (
      !isSupportedGithubUrl(document?.location?.href) ||
      !selection ||
    ...
    const singleEndpointSide =
      endpointSides.length === 2 && endpointSides[0] === endpointSides[1]
        ? endpointSides[0]
        : undefined
    const rows = [
      ...(document.querySelectorAll?.(
        ".blob-code.js-file-line, [data-line-anchor][data-line-number]"
      ) ?? []),
    ].filter((node) => {
    ...
    const locations = rows
      .map((row) => {
        ...
        const deleted =
          ...
          (splitSide == null && row.querySelector?.(".deletion"))
        const added =
          row.classList?.contains("blob-code-addition") ||
          row.classList?.contains("addition") ||
          (splitSide == null && row.querySelector?.(".addition"))
        const version =
          diffSide === "left" || deleted
            ? "original"
            : diffSide === "right" || added
              ? "modified"
              : "both"
        const sideCell =
          diffSide === "left"
        ...
        const line =
          ...
          afterCell?.getAttribute("data-line-number")
        const anchor =
          row.getAttribute?.("data-line-anchor") ??
      ...
      .filter(
        (location) =>
          ...
          Number.isInteger(Number(location.line))
      )
    ...
    const lines =
      versions.length === 1
        ...
        : versions
            .map(
              (version) =>
                `${version} ${lineRange(locations.filter((location) => location.version === version).map((location) => location.line))}`
            )
            .join("; ")
    ...
    return {
      file: files[0],
      version:
        versions.length === 1 && versions[0] === "both"
  ...
  function getDiffSide(node) {
    const directSide =
      node?.getAttribute?.("data-diff-side") ?? node?.getAttribute?.("data-split-side")
    if (directSide != null) {
      return directSide
    }

    const sideElement =
      node?.closest?.("[data-diff-side], [data-split-side]") ??
      node?.parentElement?.closest?.("[data-diff-side], [data-split-side]")
    return (
      sideElement?.getAttribute?.("data-diff-side") ??
      sideElement?.getAttribute?.("data-split-side")
    )
  }
  ...
  function diffFileFromRow(row) {
    ...
    return typeof label === "string" && label.startsWith("Diff for: ")
      ? label.slice("Diff for: ".length)
      : undefined
  }
  ...
  function extractGithubCommit(document) {
    const headAttributes = [
      ...(document.querySelectorAll?.(
        "[data-head-sha], [data-head-commit-sha], [data-pull-request-head-sha]"
      ) ?? []),
    ].flatMap((node) => [
  ...
  function lineRange(lines) {
    ...
    if (
      values.length === 0 ||
      values.some((value, index) => index > 0 && value !== values[index - 1] + 1)
    ) {
      return undefined
    }
  ...
  function buildDiffPermalink(document, locations) {
    ...
    if (
      !first ||
      parsed.some((match) => !match || match[1] !== first[1] || match[2] !== first[2])
    ) {
      return undefined
    }
  ...
  function createEnvelope({ selectionText, linkUrl, pageUrl, title, diffLocation }) {
    ...
    if (typeof selectionText !== "string" || selectionText.trim().length === 0) {
      throw new Error(
        isSupportedGithubUrl(pageUrl)
          ? "Select GitHub code before sending context to OMP."
          : "Select text before sending context to OMP."
      )
    }

    const github = isSupportedGithubUrl(pageUrl)
    const url =
      github && isHttpUrl(diffLocation?.permalink)
        ? diffLocation.permalink
        : github && isHttpUrl(linkUrl)
          ? linkUrl
          : pageUrl
    return {
      version: 1,
      ...
      prompt: formatPrompt({
        ...
        title,
        github,
        diffLocation: github ? diffLocation : undefined,
      }),
      metadata: {
        url,
  ...
  function formatPrompt({ selectionText, url, title, github = true, diffLocation }) {
    ...
    if (github && diffLocation?.file) {
      ...
      if (diffLocation.lines) {
        const readableLines =
          diffLocation.version === "original and modified"
  ...
  function normalizeSelectionText(text) {
    ...
    return [
      ...(leadingBlank ? [""] : []),
      ...content.filter((_, index) => index % 2 === 0),
      ...(trailingBlank ? [""] : []),
    ].join("\n")
  }
