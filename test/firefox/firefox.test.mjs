...
const contextSource = await fs.readFile(
  new URL("../../firefox/context.js", import.meta.url),
  "utf8"
)
...
const manifest = JSON.parse(
  await fs.readFile(new URL("../../firefox/manifest.json", import.meta.url), "utf8")
)
const firefoxPackage = JSON.parse(
  await fs.readFile(new URL("../../firefox/package.json", import.meta.url), "utf8")
)
const context = { URL }
...
test("Firefox client recognizes eligible web pages", () => {
  ...
  assert.equal(
    isEligiblePageUrl("https://addons.mozilla.org/en-CA/firefox/addon/omp-send-context/"),
    false
  )
...
test("Firefox manifest declares branded icons and supported desktop metadata", async () => {
  ...
  for (const iconSize of ["48", "96", "128"]) {
    const bytes = await fs.readFile(
      new URL(`../../firefox/icons/icon-debug-${iconSize}.png`, import.meta.url)
    )
...
test("Firefox extracts GitHub diff file, side, and contiguous lines", () => {
  const makeRow = (line, side) => {
    const code = {
      classList: {
        contains: (name) => name === "blob-code-" + (side === "after" ? "addition" : "deletion"),
      },
      closest: (selector) =>
        selector === "[data-tagsearch-path]" ? { getAttribute: () => "src/example.ts" } : tr,
      selected: true,
    }
    const tr = {
      querySelector: (selector) => ({
        getAttribute: (name) =>
          name === "data-line-number" && selector.includes("addition") ? String(line) : undefined,
      }),
    }
    return code
  }
...
test("Firefox builds exact GitHub diff permalinks for both sides", () => {
    ...
    const cell = {
      getAttribute: (name) =>
        name === "data-line-number"
          ? String(line)
          : `diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1${side === "after" ? "R" : "L"}${line}`,
    }
    const code = {
      classList: {
        contains: (name) => name === `blob-code-${side === "after" ? "addition" : "deletion"}`,
      ...
      closest: (selector) =>
        selector === "[data-tagsearch-path]"
          ...
          : {
              querySelector: (query) =>
                query.includes(side === "after" ? "addition" : "deletion") ? cell : undefined,
            },
    }
  ...
  assert.equal(
    after.permalink,
    "https://github.com/icefoganalytics/wrap/pull/490/changes#diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1R8"
  )
  assert.equal(
    before.permalink,
    "https://github.com/icefoganalytics/wrap/pull/490/changes#diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1L8"
  )
  assert.match(
    createEnvelope({
    ...
    /- Location: https:\/\/github\.com\/icefoganalytics\/wrap\/pull\/490\/changes#diff-.*R8/
  )
})
...
test("Firefox builds a GitHub diff range permalink for contiguous same-side selections", () => {
  ...
  assert.equal(
    location.permalink,
    "https://github.com/org/repo/pull/42/files#diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1R8-R9"
  )
})
...
test("Firefox extracts human-readable metadata from the current GitHub diff DOM", () => {
    ...
    closest: (selector) =>
      selector === "table"
        ? {
            getAttribute: (name) =>
              name === "aria-label"
                ? "Diff for: web/src/components/dashboards/DashboardTitleRow.vue"
                : undefined,
          }
        : undefined,
  ...
  const document = {
    location: { href: "https://github.com/icefoganalytics/wrap/pull/490/changes" },
    querySelectorAll: (selector) =>
      selector.includes("data-head-sha")
        ? [
            {
              getAttribute: (name) =>
                name === "data-head-sha"
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(location)), {
    ...
    lines: "8-9",
    permalink:
      "https://github.com/icefoganalytics/wrap/pull/490/changes#diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1R8-R9",
  ...
  const row = {
    ...
    getAttribute: (name) =>
      ({
        ...
        "data-line-number": "8",
        "data-line-anchor":
          "diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1R8",
      })[name],
    closest: (selector) =>
      selector === "table"
        ? {
...
test("Firefox keeps an original-only selection on the original side", () => {
  const rows = [8, 9, 10, 11, 12, 13, 14, 15, 16]
    .map((line) => ["left", line])
    .concat([8, 9, 10, 11, 12, 13, 14, 15].map((line) => ["right", line]))
    .map(([side, line]) => ({
      classList: { contains: () => false },
      getAttribute: (name) =>
        ({
  ...
  const document = {
    location: { href: "https://github.com/icefoganalytics/wrap/pull/490/changes" },
    querySelectorAll: (selector) =>
      selector.includes("data-head-sha") || selector.includes("data-commit") ? [] : rows,
  }
  const selection = {
    rangeCount: 1,
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(location)), {
    ...
    lines: "8-16",
    permalink:
      "https://github.com/icefoganalytics/wrap/pull/490/changes#diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1L8-L16",
  })
  assert.match(
    formatPrompt({
    ...
    .map(([side, line]) => {
      const leftCell = {
        getAttribute: (name) =>
          name === "data-line-number"
            ? String(line)
            : `diff-c4ff09aa6de01afd7b040b9c957c2c1de47a029f70e3bade00120be8caa8daf1L${line}`,
      }
      const rightCell = {
        getAttribute: (name) =>
          name === "data-line-number"
...
test("Firefox classifies a split row from the selected side", () => {
  const makeRow = (side, change) => {
    ...
    const rightCell = {
      getAttribute: (name) => (name === "data-line-number" ? "8" : "diff-fileR8"),
    }
    return {
      getAttribute: (name) => (name === "data-split-side" ? side : undefined),
      ...
      closest: (selector) =>
        selector === "tr"
          ...
          : selector === "[data-tagsearch-path]"
            ...
            : undefined,
      querySelector: (selector) =>
        selector === ".deletion" || selector === ".addition" ? {} : undefined,
    }
  }
  ...
  const makeRow = (side, change) => ({
    ...
    closest: (selector) =>
      selector === "table"
        ? {
            getAttribute: (name) =>
              name === "aria-label" ? "Diff for: src/example.ts" : undefined,
          }
        : undefined,
    ...
    closest: (selector) =>
      selector === "[data-tagsearch-path]"
        ...
        : {
            querySelector: (query) => ({
              getAttribute: (name) =>
                name === "data-line-number" && query.includes("deletion") ? "42" : undefined,
            }),
          },
  }
  const location = extractGithubDiffLocation(
  ...
  const location = extractGithubDiffLocation(
    {
      location: { href: "https://github.com/org/repo/pull/42/files" },
      querySelectorAll: (selector) =>
        selector.includes("data-head-sha") || selector.includes("data-commit") ? [] : [code],
    },
    { rangeCount: 1, getRangeAt: () => ({ intersectsNode: () => true }) }
  )
  assert.deepEqual(JSON.parse(JSON.stringify(location)), {
    ...
    closest: (selector) =>
      selector === "[data-tagsearch-path]"
        ...
        : {
            querySelector: (query) => ({
              getAttribute: (name) =>
                name === "data-line-number" && query.includes("addition")
                  ? String(line)
                  : undefined,
            }),
          },
...
test("Firefox content capture returns diff location metadata", async () => {
  ...
  assert.deepEqual(capture.diffLocation, {
    file: "src/example.ts",
    version: "modified",
    change: "added",
    lines: "124",
