...
import {
  ...
  deliverEnvelope,
  encodeMessage,
} from "../../firefox/native-host/omp-send-context-host.mjs"
...
test("native host rejects invalid envelopes and non-loopback bridge state", async () => {
  ...
  assert.throws(
    () => assertEnvelope({ version: 1, source: "firefox", prompt: "x", metadata: [] }),
    /object metadata/
  )
  assert.throws(
    () => assertEnvelope({ version: 1, source: "firefox", prompt: "x".repeat(2_100_000) }),
    /too large/
  )
  ...
  try {
    await fs.writeFile(
      stateFile,
      JSON.stringify({ endpoint: "https://example.com", token: "secret" })
    )
